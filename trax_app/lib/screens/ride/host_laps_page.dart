...
/// Host Laps — multi-rider lap-timer session.
/// Same lobby/lifecycle as a Race (uses the shared Race backend with
/// `gameType=LAPS`). Each rider configures their own checkpoints during the
...
class _HostLapsPageState extends State<HostLapsPage> {
  ...
  Future<void> _onCreate() async {
    if (_nameCtrl.text.trim().isEmpty) {
      showTraxSnackBar(context, 'Please enter a session name', isError: true);
      return;
    }
  ...
  Widget _buildContent(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: traxTitle('Host Laps')),
      body: _loading
          ? const Center(
          ...
          : ListView(
              ...
              children: [
                ...
                _buildScheduleRow(),
                const SizedBox(height: 16),
                _buildCounterRow('Max Participants', _maxParticipants, 2, 50,
                    (v) => setState(() => _maxParticipants = v)),
                const SizedBox(height: 16),
                _buildCounterRow('Target Laps', _targetLaps, 1, 100,
                    (v) => setState(() => _targetLaps = v)),
                const SizedBox(height: 16),
                _buildToggleRow(),
                const SizedBox(height: 16),
                _buildTextField('Notes (optional)', _notesCtrl,
                    'Any extra info…',
                    maxLines: 3),
  ...
  Widget _buildCreateButton() {
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: const Text('Create Laps Session',
