...
/// 本文件基于 PrettyDioLogger
///
/// 当 Response data 是 Map 的时候，对 data 做了 json格式化，
/// 不然看不出数据类型
///
...
class AppLoggerInterceptor extends Interceptor {
  ...
  void _printBoxed({String? header, String? text}) {
    ...
    logPrint('');
    logPrint('╔╣ $header ║ DateTime: $nowTime');
    logPrint('║  $text');
    _printLine('╚');
  }
  ...
  void _printResponse(Response response) {
    if (response.data is Map) {
      String text = JsonEncoder.withIndent(_indent()).convert(response.data);
      text.split('\n').forEach((v) => _printBlock(v));
